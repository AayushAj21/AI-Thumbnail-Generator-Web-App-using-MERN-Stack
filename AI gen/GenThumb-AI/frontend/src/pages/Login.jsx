  ...
  return (
    <div className="py-20 px-6 flex items-center justify-center">
      <div className="max-w-md w-full bg-dark-800 p-8 rounded-3xl border border-white/5 shadow-2xl relative overflow-hidden group">
        ...
        <div className="absolute top-0 right-0 -mr-16 -mt-16 w-32 h-32 rounded-full bg-brand-500/20 blur-2xl"></div>
        
        <div className="text-center mb-8 relative z-10">
          <h2 className="text-3xl font-extrabold text-white">Welcome Back</h2>
          <p className="text-gray-400 mt-2">Log in to your GenThumb account</p>
        </div>
        ...
        <form onSubmit={submitHandler} className="space-y-6 relative z-10">
          ...
          <button 
            type="submit" 
            disabled={loading}
            className={`w-full py-3 px-4 bg-brand-600 hover:bg-brand-500 text-white font-bold rounded-xl shadow-lg shadow-brand-500/30 transition transform hover:-translate-y-0.5 flex items-center justify-center gap-3 ${loading ? 'opacity-70 cursor-not-allowed' : ''}`}
          >
        ...
        <p className="mt-8 text-center text-gray-400 text-sm">
          Don't have an account? <Link to="/signup" className="text-brand-500 hover:text-brand-400 font-semibold transition">Sign up free</Link>
        </p>
      </div>
    </div>
  );
